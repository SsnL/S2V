...
from bc_kfold import KFold


def eval_nested_kfold(encoder, name, loc='./data/', k=10, seed=1234, use_nb=False, just_one=False):
    """
    ...
    for train, test in kf:
        ...
        for s in scan:
            ...
            for innertrain, innertest in innerkf:
                ...
                # Train classifier
                clf = LogisticRegression(C=s, solver='liblinear')
        ...
        print ('train', tr_scores)
        print ('test', te_scores)

        if just_one:
            break

    return dict(train=tr_scores, test=te_scores)
